...
abstract class AbstractWeiboOperations {
    ...
    protected AbstractWeiboOperations(ObjectMapper objectMapper, RestTemplate restTemplate, boolean isAuthorized) {
    ...
    protected <T> CursoredList<T> deserializeCursoredList(JsonNode jsonNode, final Class<T> elementType,
        ...
        result.addAll(deserializeDataList(jsonNode.get(dataFieldName), elementType));
        return result;
    }

    @SuppressWarnings("unchecked")
    protected <T> List<T> deserializeDataList(JsonNode jsonNode, final Class<T> elementType) {
        try {
            CollectionType listType = TypeFactory.defaultInstance().constructCollectionType(List.class, elementType);
            return (List<T>) objectMapper.reader(listType).readValue(jsonNode.toString());
        } catch (IOException e) {
            throw new UncategorizedApiException("weibo", "Error deserializing data from Weibo: " + e.getMessage(), e);
        }
    }
